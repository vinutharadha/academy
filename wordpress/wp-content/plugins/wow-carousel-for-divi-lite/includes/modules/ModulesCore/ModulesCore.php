...
class WDCL_Builder_Module extends ET_Builder_Module {
	 ...
	 * @param  string $toggle_slug
	 * @param  array $background_tab
	 *
	 * @return array
	 */
	...
	/**
	 * Carouse Option Settings Fields
	 *
	 * @param $design_slug Carousel Design toggle_slug
	 * @return array
	 */
    ...
    protected function _render_pagination_css( $render_slug ) {
		...
		\ET_Builder_Element::set_style( $render_slug, array(
			...
			'declaration' => sprintf(' background: %1$s; height: %2$s; width: %3$s; border-radius: %4$s %5$s %6$s %7$s;',
				$pagi_color,
				$pagi_height,
				$pagi_width,
				$pagi_radius[1],
	...
	protected function _render_carousel_css( $render_slug ) {
		...
		\ET_Builder_Element::set_style( $render_slug, array(
			'selector'    => '%%order_class%% .slick-arrow',
			'declaration' => sprintf( 'height: %1$s; width: %2$s; color: %3$s; background: %4$s; border: %5$s %6$s %7$s; transform: skew(%8$s);margin-top:-%9$spx;',
				$arrow_height,
				$arrow_width,
				$arrow_color,
				$arrow_bg,
		...
		\ET_Builder_Element::set_style( $render_slug, array(
			...
			'declaration' => sprintf( 'font-size: %1$s; transform: skew(%2$sdeg); display: inline-block;',
				$arrow_icon_size, $arrow_skew_inner
			),
		) );
    ...
    /**
     * Carousel Options For Front End
     * @return void
     */
